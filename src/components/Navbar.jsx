        ...
        <nav className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4 md:px-12">
          ...
          <div className="hidden md:flex gap-10 text-xs uppercase tracking-wider">
            {["about", "whatwedo", "collab"].map(id => (
              <a key={id} href={`#${id}`} onClick={(e)=>handleNavClick(e,id)} className="text-white/70 hover:text-primary transition">
      ...
      <div className={`fixed inset-0 bg-black/95 backdrop-blur-xl z-[999] transition-all duration-500 md:hidden ${isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-6 text-base uppercase tracking-widest">
          {["about","whatwedo","partners"].map(id=>(
            <a key={id} href={`#${id}`} onClick={(e)=>handleNavClick(e,id)} className="text-white/90 hover:text-primary transition">
              {id === "whatwedo" ? "What We Do" : id}
            </a>
          ))}
          <Link to="/careers" onClick={()=>setIsOpen(false)} className="text-white/90 hover:text-primary">Careers</Link>
          <a href="#contact" onClick={(e)=>handleNavClick(e,"contact")} className="mt-6 px-10 py-3 border border-primary/40 text-primary text-xs font-bold tracking-widest rounded-full hover:bg-primary hover:text-black transition">
            Build With Us
          </a>
        </div>
      </div>
